...
def get(task_id: int, db_path: Path = DEFAULT_DB):
    with connect(db_path) as conn:
        return conn.execute(
            "SELECT id, title, status, created_at, completed_at, archived_at, priority, due_date, estimate_minutes, billable FROM tasks WHERE id=?",
            (task_id,),
        ).fetchone()
...
def list_tasks(
    ...
    *,
    ...
    """
    Return tasks. If tags specified, tasks must have ALL listed tags.
    """
    tags = tags or []
    q = """
...
def merge_tasks(src_task_id: int, dst_task_id: int, db_path: Path = DEFAULT_DB) -> bool:
    """Move logs and tags from src → dst and delete src."""
    with connect(db_path) as conn:
        # reassign entries
